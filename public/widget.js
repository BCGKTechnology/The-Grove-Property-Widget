...
(function () {
  ...
  // ---------------------------------------------------------------------
  // Guarantee a correct mobile viewport, regardless of the host page.
  // ---------------------------------------------------------------------
  // Every @media (max-width: ...) rule below only works correctly if the
  // page declares <meta name="viewport" content="width=device-width, ...">.
  // Without it, mobile Safari/Chrome fall back to treating the page as a
  // 980px-wide desktop layout and then auto-zoom the ENTIRE page out to fit
  // the real screen (e.g. ~45% zoom on a 440px-wide iPhone) — our media
  // queries can still technically match (via the pointer:coarse check), but
  // everything they size (the FAB, tap targets, fonts) gets rendered at
  // roughly half scale because the whole page is shrunk, not just measured
  // correctly. This was diagnosed directly: on a real iPhone-sized viewport
  // with no viewport meta tag, window.visualViewport.scale came back as
  // ~0.45 instead of 1. We don't control RentCafe's page <head>, so rather
  // than assume it has this tag, we add it ourselves if it's missing —
  // this is a page-wide fix (not just our widget) but is exactly what a
  // correctly-configured mobile page should have anyway, and we only add
  // it when one isn't already present so we never override a real one.
  try {
    ...
    /* non-fatal — worst case the host page's own viewport handling applies */
  }
  ...
  style.textContent = `
    ...
    * { box-sizing: border-box; font-family: ${CONFIG.theme.fontFamily}; }
    ...
    /* Mobile: a genuine full-screen sheet, not a shrunk-down desktop card
       parked in the corner. Width AND coarse-pointer are both checked
       (comma = OR) since some in-app browsers (e.g. an iMessage link
       preview) can report a viewport wider than the physical phone screen,
       so pointer type is a more reliable phone/tablet signal than width
       alone in those contexts. Breakpoint is intentionally generous
       (768px) to also cover tablets and any in-between test viewport. */
    @media (max-width: 768px), (hover: none) and (pointer: coarse) {
      ...
      /* height (not just max-height) is what actually makes this a full
         sheet — max-height alone only caps a box that still shrinks to
         fit its own content, which is why a short form previously left
         the hero image showing above a half-height card. Setting an
         explicit height forces the sheet to occupy the screen regardless
         of how much content is inside it, with the body below scrolling
         internally if needed. 100dvh (dynamic viewport height) accounts
         for mobile Safari's collapsing address bar; 100vh is the fallback
      ...
      .panel-header { padding: calc(20px + env(safe-area-inset-top)) 20px 18px; }
      .panel-header h2 { font-size: 21px; }
      .panel-header p { font-size: 14.5px; }
      .panel-header.plain h2 { font-size: 19px; }

      .icon-btn { top: calc(16px + env(safe-area-inset-top)); padding: 12px; }
      .icon-btn svg { width: 24px; height: 24px; }
      .close-btn { right: 8px; }
      .back-btn { left: 8px; }

      /* flex:1 is what lets this actually fill the rest of the sheet
         (rather than shrink-wrapping its content) — panel is already a
         flex column, so the header takes its natural height and this
         takes everything left over, scrolling on its own if content runs
         long. The 36px+ bottom padding keeps the last CTA button clear of
         the thumb/home-indicator area rather than sitting flush against
         the bottom edge. */
      .panel-body {
        flex: 1;
        display: flex;
      ...
      .tile { padding: 18px; font-size: 16.5px; min-height: 60px; }
      .tile .tile-icon { width: 42px; height: 42px; }
      .tile .tile-icon svg { width: 21px; height: 21px; }

      label { font-size: 14px; }
      /* 16px stops iOS Safari from auto-zooming the page when a field is
         focused — a mobile-specific fix, not just a size preference.
         55px min-height is a comfortable thumb target, well above the
         44px minimum most platform guidelines call for. */
      input, select, textarea {
        font-size: 16px;
        padding: 15px 14px;
        min-height: 55px;
      ...
      .submit-btn { padding: 16px; font-size: 16.5px; min-height: 56px; margin-top: 8px; }

      .send-text-row input { min-height: 55px; font-size: 16px; }
      .send-text-row button { padding: 0 22px; font-size: 14.5px; min-height: 55px; }

      .contact-phone a { font-size: 19px; }
      .hours-table td { font-size: 15px; padding: 8px 0; }

      .consent { font-size: 13px; }
    }

    /* Smallest phones only: stack paired fields (first/last name, date/time)
       instead of squeezing two inputs into one row. */
